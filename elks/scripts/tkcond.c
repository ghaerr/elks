 ...
 *
 * This file contains the code to further process the conditions from the
 * "ifdef" clauses.
 *
...
/*
 * Walk a condition chain and invert it so that the logical result is inverted.
 */
static void invert_condition(struct condition *cnd)
{
    /*
     * Just walk through the list and invert all of the operators.
     */
	...
	case op_or:
	    /*
	     * This is not turned into op_and - we need to keep track of what
	     * operators were used here since we have an optimization later
	     * on to remove duplicate conditions, and having inverted ors in
	     * there would make it harder if we did not distinguish an
	     * inverted or from an and we inserted because of nested ifs.
	     */
	    cnd->op = op_and1;
	    break;
	case op_neq:
...
/*
 * Walk all of the conditions, and look for choice values.
 * Convert the tokens into something more digestible.
 */
 ...
 * gluing them together. The conditions from each level of the stack are
 * wrapped in parenthesis so as to guarantee that the results are logically
 * correct.
 */
...
/*
 * Walk a single chain of conditions and clone it. These are assumed to
 * have been created/processed by get_token_cond in a previous pass.
 */
...
void fix_conditionals(struct kconfig *scfg)
{
    ...
    /*
     * Start by walking the chain. Every time we see an ifdef, push the
     * condition chain on the stack. When we see an "else", we invert the
     * condition at the top of the stack, and when we see an "endif" we
     * free all of the memory for the condition at the top of the stack
     * and remove the condition from the top of the stack.
     *
     * For any other type of token (i.e. a bool), we clone a new condition
     * chain by anding together all of the conditions that are currently
     * stored on the stack. In this way, we have a correct representation
     * of whatever conditions govern the usage of each option.
     */
	...
	case tok_else:
	    /*
	     * For an else, we just invert the condition at the top of the
	     * stack. This is done in place with no reallocation of memory
	     * taking place.
	     */
	...
	case tok_choice:
	    /*
	     * We need to duplicate the chain of conditions and attach them
	     * to this token.
	     */
	...
	case tok_dep_tristate:
	     ...
	     * conditional. (Sort of a hybrid tok_if, tok_tristate,
	     * tok_fi option)
	     */
	    conditions[depth] = cfg->cond;
	    depth++;
    ...
    /*
     * Walk through and see if there are multiple options that control the same
     * kvariable. If there are we need to treat them a little bit special.
     */
    for (cfg = scfg; cfg != NULL; cfg = cfg->next) {
	switch (cfg->tok) {
	case tok_bool:
	...
	for (cnd = cfg->cond; cnd; cnd = cnd->next) {
	    /*
	     * Now search the condition list for a known configuration
	     * variable that has conditions of its own.
	     */
	    if (cnd->op != op_kvariable)
		continue;
	    if (cnd->variable.cfg->cond == NULL)
		continue;

	    if (cnd->variable.cfg->flags & CFG_DUP)
		continue;
	    /*
	     * OK, we have some conditions to append to cfg.
	     * Make a clone of the conditions,
	     */
    ...
    /*
     * There is a strong possibility that we have duplicate conditions in here.
     * It would make the script more efficient and readable to remove these.
     * Here is where we assume here that there are no parenthesis in the input
     * script.
     */
    for (cfg = scfg; cfg != NULL; cfg = cfg->next) {
	/*
	...
	for (cnd = cfg->cond; cnd; cnd = cnd->next) {
		...
		/*
		 * Search after the previous left parenthesis,
		 * and try to find a second left parenthesis.
		 */
		if (cnd1->op != op_lparen)
		    continue;

		/*
		 * Now compare the next 5 tokens to see if they are identical.
		 * We are looking for two chains that are like:
		 *
		...
		for (i = 0; i < 5; i++, cnd2 = cnd2->next, cnd3 = cnd3->next) {
		    ...
		    if (i == 1 && (cnd2->op != op_kvariable
				   || cnd2->variable.cfg !=
				   cnd3->variable.cfg)) break;
		    if (i == 2 && cnd2->op != op_eq && cnd2->op != op_neq)
			break;
		    if (i == 3 && cnd2->op != op_constant &&
			strcmp(cnd2->variable.str, cnd3->variable.str) != 0)
			break;
		    if (i == 4 && cnd2->op != op_rparen)
			break;
		}
		/*
		...
		if (i == 5 && ((cnd3 && cnd3->op == op_and)
			       || (cnd2 && cnd2->op == op_and))) {
		    /*
		     * We have a duplicate. Nuke 5 operations.
		     */
		    cnd3 = cnd1;
		    ...
		    /*
		     * Nuke the and that glues those conditions together.
		     */
		    if (cnd3 && cnd3->op == op_and)
			cnd3->op = op_nuked;
		    else if (cnd2 && cnd2->op == op_and)
			cnd2->op = op_nuked;
		}
	    }
	}
