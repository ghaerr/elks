...
#ifdef BLOAT_FS
unsigned long elksfs_count_free_blocks(register struct super_block *sb)
{
    return (sb->u.elksfs_sb.s_nzones -
	    count_used(sb->u.elksfs_sb.s_zmap,
		       sb->u.elksfs_sb.s_zmap_blocks,
...
void elksfs_free_inode(register struct inode *inode)
{
#ifdef CONFIG_FS_RO
    clear_inode(inode);
#else
...
#ifndef CONFIG_FS_RO
struct inode *elksfs_new_inode(struct inode *dir)
{
...
#ifdef BLOAT_FS
unsigned long elksfs_count_free_inodes(register struct super_block *sb)
{
    return sb->u.elksfs_sb.s_ninodes
	- count_used(sb->u.elksfs_sb.s_imap,
		     sb->u.elksfs_sb.s_imap_blocks, sb->u.elksfs_sb.s_ninodes);
}
#endif
