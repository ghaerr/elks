...
static void select_callback(unused)
	unsigned long unused;
{
	floppy_ready();
}

static void floppy_select(nr)
	unsigned int nr;
{
#ifndef TRP_TIMER
...
static void motor_on_callback(nr)
	unsigned long nr;
{
	running |= 0x10 << nr;
	floppy_select(nr);
...
#endif
static void motor_off_callback(nr)
	unsigned long nr;
{
 ...
 */
static void perpendicular_mode(rate)
	unsigned char rate;
{
 ...
 */
static void configure_fdc_mode()
{
...
static void shake_zero()
{
	if (!retry_recal(shake_zero)) shake_done();
}

static void shake_one()
{
...
static int fd_ioctl(inode,filp,cmd,param)
	struct inode *inode;
	struct file *filp;
	unsigned int cmd;
	unsigned long param;
...
static struct floppy_struct *find_base(drive,code)
	int drive;
	int code;
{
 ...
 */
static int floppy_open(inode,filp)
	struct inode * inode;
	struct file * filp;
{
	int drive;
...
static void floppy_release(inode,filp)
	struct inode * inode;
	struct file * filp;
{
...
static void floppy_interrupt(unused)
	int unused;
{
