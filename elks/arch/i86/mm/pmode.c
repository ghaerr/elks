...
void gdt_init(void)
{
    ...
    /* A zero-sized, unused track buffer must not underflow desc_set(). */
#if TRACKSEGSZ
    ...
    /* Shared low-memory DMA bounce buffer. */
    desc_set(MK_SEL(GDT_DMABUF, SEL_GDT, SEL_RPL0), SEG_DMASEG << 4,
        DMASEGSZ ? DMASEGSZ : 512, DESC_KDATA);
