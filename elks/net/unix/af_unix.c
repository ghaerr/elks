...
static struct unix_proto_data * unix_data_alloc()
{
	...
	for(upd = unix_datas; upd <= last_unix_data; ++upd) {
		if (!upd->refcnt) {
			upd->refcnt = -1;       /* unix domain socket not yet in
			itialised - bgm */
			isti();
			upd->socket = NULL;
			upd->sockaddr_len = 0;
...
static struct unix_proto_data * unix_data_lookup(sockun, sockaddr_len, inode)
...
static void unix_data_deref(upd)
struct unix_proto_data *upd;
{
	if (upd->refcnt == 1) {
		upd->bp_head = upd->bp_tail = 0;
...
static int unix_dup(newsock, oldsock)
...
static int unix_release(sock, peer)
struct socket *sock;
struct socket *peer;
{
...
static int unix_bind(sock, umyaddr, sockaddr_len)
struct socket *sock;
...
static int unix_connect(sock, uservaddr, sockaddr_len, flags)
struct socket *sock;
...
static int unix_socketpair(sock1, sock2)
struct socket *sock1;
struct socket *sock2;
{
...
static int unix_accept(sock, newsock, flags)
struct socket *sock;
struct socket *newsock;
int flags;
{
...
static int unix_getname(sock, usockaddr, usockaddr_len, peer)
struct socket *sock;
...
static int unix_read(sock, ubuf, size, nonblock)
struct socket *sock;
char *ubuf;
int size;
int nonblock;
...
static int unix_write(sock, ubuf, size, nonblock)
struct socket *sock;
char *ubuf;
int size;
int nonblock;
...
static int unix_select(sock, sel_type, wait)
struct socket *sock;
int sel_type;
select_table * wait;
{
...
static int unix_listen(sock, backlog)
struct socket *sock;
int backlog;
{
	return(0);
...
static int unix_send(sock, buff, len, nonblock, flags)
struct socket *sock;
void *buff;
int len;
int nonblock;
...
static int unix_recv(sock, buff, len, nonblock, flags)
struct socket *sock;
void *buff;
int len;
int nonblock;
