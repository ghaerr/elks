...
static int  HERC_open(PSD psd);
static void HERC_close(PSD psd);
static void HERC_getscreeninfo(PSD psd,PSCREENINFO psi);;
static void HERC_setpalette(PSD psd,int first,int count,RGBENTRY *pal);
static void HERC_drawpixel(PSD psd,COORD x, COORD y, PIXELVAL c);
static PIXELVAL HERC_readpixel(PSD psd,COORD x, COORD y);
static void HERC_drawhline(PSD psd,COORD x1, COORD x2, COORD y, PIXELVAL c);
static void HERC_drawvline(PSD psd,COORD x, COORD y1, COORD y2, PIXELVAL c);
static void HERC_fillrect(PSD psd,COORD x1,COORD y1,COORD x2,COORD y2,
		PIXELVAL c);
static void HERC_blit(PSD dstpsd,COORD destx,COORD desty,COORD w,COORD h,
