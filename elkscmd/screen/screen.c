...
int
MakeWindow(prog, args, aflag, StartAt, dir)
    char *prog, **args, *dir;
{
    register struct win **pp, *p;
    register char **cp;
...
static SendCreateMsg(s, ac, av, aflag)
char **av; {
    struct msg m;
    register char *p;
    register len, n;
...
static char **
SaveArgs(argc, argv) register argc;
    register char **argv;
{
    register char **ap, **pp;
...
static IsNum(s, base) register char *s;
register base; {
    for (base += '0'; *s; ++s)
        if (*s < '0' || *s > base)
            return 0;
