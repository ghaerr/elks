...
static int hasattr(lno, text)
	...
	char	*text;	/* the text of the line, from fetchline */
{
	...
	int		inputting;	/* boolean: called from input() ? */
{
	...
	static long	chgs;	/* previous value of changes counter */
	int		col;	/* used to count column numbers */
	int		idx;	/* used to count down the index */
	int		i;
...
{
	REG char		*text;		/* a line of text to display */
