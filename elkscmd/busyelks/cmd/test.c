...
register char *s;
{
  register struct op *op = test_ops;
...
int n;
{
  ...
  if (n == EOI) syntax();
  if (n == UNEGN) return (!expr(test_lex(*++ip)));
  if (n == LPAREN) {
	res = expr(test_lex(*++ip));
	if (test_lex(*++ip) != RPAREN) syntax();
	return(res);
  }
  if (n == OPERAND) {
	opnd1 = *ip;
...
int n;
{
  ...
  if (n == EOI) syntax();
  res = primary(n);
  if (test_lex(*++ip) == BAND) return(bexpr(test_lex(*++ip)) && res);
  ip--;
  return(res);
}
...
int n;
{
  ...
  if (n == EOI) syntax();
  res = bexpr(n);
  if (test_lex(*++ip) == BOR) return(expr(test_lex(*++ip)) || res);
  ip--;
  return(res);
}
