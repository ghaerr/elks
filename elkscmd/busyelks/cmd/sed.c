...
/* 2012-02-22: Jody Bruchon: Changed quit() to exit() */
...
_PROTOTYPE(void sed_execute, (void));
...
char *argv[];
{
  ...
  if (eargc == 1) exit(0);	/* exit immediately if no arguments */
  ...
  *cbuf-- = 0;			/* so pre-increment points us at cbuf */
  ...
  if (eflag) {
	...
	if (eflag > 0) {	/* there are pending -e arguments */
		eflag = -1;
		if (eargc-- <= 0) exit(2);	/* if no arguments, barf */
