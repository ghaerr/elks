 ...
 */
volatile void fsck_fatal_error(const char * fmt_string, int status)
{
	fprintf(stderr,fmt_string,program_name,device_name);
	exit(status);
}

#define usage() fsck_fatal_error("Usage: %s [-larvsmf] /dev/name\n",16)
#define die(str) fsck_fatal_error("%s: " str "\n",8)
 ...
 */
void fsck_write_block(unsigned int nr, char * addr)
{
	off_t dest = (off_t)BLOCK_SIZE * nr;
	if (!nr)
		return;
...
unsigned int map_block(struct minix_inode * inode, unsigned int blknr)
{
	...
	if (blknr<512) {
		...
		if (blk_chg)
			fsck_write_block(block, (char *) ind);
		return result;
	}
	printd(")dind\n");
	blknr -= 512;
	...
	if (blk_chg)
		fsck_write_block(block, (char *) dind);
	block = result;
	read_block(block, (char *) ind);
	blk_chg = 0;
...
void fsck_check_file(struct minix_inode * dir, unsigned long offset)
{
	static char blk[BLOCK_SIZE];
	struct minix_inode * inode;
	int ino;
...
int fsck_main(int argc, char ** argv)
{
	...
	check_root();
	/* Next 6 lines used to be a pointless check() function */
	printd("check\n");
