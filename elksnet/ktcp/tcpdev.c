...
static void tcpdev_bind()
{
	...
	if(port == 0){
		if(next_port < 1024)next_port = 1024;
...
static void tcpdev_accept()
{
	struct tcpcb_list_s *n,*newn;
	struct tdb_accept *db;
...
static void tcpdev_release()
{
	struct tdb_release *db;
   	struct tcpcb_list_s *n;
   	struct tcpcb_s *cb;
	__u16  sock;
...
void tcpdev_process()
{
    ...
    len = read(tcpdevfd, sbuf, TCPDEV_BUFSIZE);
	if(len <= 0)return;
