...
configuration, that folder is used to create a floppy disk image
...
6- Optionally build external applications. (This requires OpenWatcom v2 installed):

`./buildext.sh all`

This will use `git` to clone the external applications into `extapps/`.
Not all external applications will fit on a floppy image, most require
configuring for an HDD image for space reasons.

7- Before writing that image on the real medium, you can test it first on QEMU:

`./qemu.sh`

8- You can then modify the configuration or the sources and repeat from the
...
9- One can also build ELKS distribution images for the entire suite of
