 ...
 * cal_output() copies from layout[] to outline[], (then trims blanks).
 */
...
{
  if (year < 1 || year > 9999) cal_usage(badarg);
  if (month <= 0 || month > 12) cal_usage(badarg);
